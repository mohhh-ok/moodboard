...
- 動画(mov/webm/mp4、拡張子判定)は自動再生・ミュート・ループで表示され、上記の操作すべてが画像と同じに効く。alpha 付き待機ループ動画の見比べに使える(市松背景で透過確認)。HEVC alpha の `.mov` は再生できるが、VP9 alpha の `.webm` は再生できないことがあり、その場合は「再生不可」+ファイル名の表示になる
